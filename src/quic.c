...
static int delay_recon(aeEventLoop *loop, long long id, void *data) {
...
static size_t fd_readable(int fd) {
    int n, rc;
...
static int write_request(aeEventLoop *loop, long long id, void *data) {
    ...
    picoquic_call_back_event_t fin_or_event, void* callback_ctx, void* v_stream_ctx)
{
    ...
    int is_h3 = 0;
    if (strcmp(c->thread->alpn,"h3") == 0) {
     //   is_h3 = 1;
    }
    debug("CALLBACK %d, cx %pn h3 %d!\n",fin_or_event,cnx,is_h3);
        ...
        case picoquic_callback_prepare_to_send: {
            ...
            if (!c->written) {
                   /* if (is_h3) {
                        ...
                        &c->request, c->length, path, path_len, post_size, cfg.host, &c->length);
                        break;
                    } else*/
                     {
            ...
            uint64_t now = time_us();
            uint64_t actual_latency_timing = now - c->actual_latency_start;
            debug("RECON LAT %ld", actual_latency_timing);
...
picoquic_quic_t *quic_init() {
	    ...
	    picoquic_quic_t *quic = picoquic_create(1, NULL, NULL, NULL, ALPN_SAMPLE_SERVER, NULL, NULL,
            NULL, NULL, NULL, current_time, NULL,
            ticket_store_filename, NULL, 0);

        connection *c = 0;
        debug("Created QUIC %p", quic);
        if (quic == NULL) {
...
status quic_connect(connection *c, char *host, int fd) {
    ...
    debug("Creating context for addr %x (host %s), alpn %s, quic %p\n", sin->sin_addr, host, c->thread->alpn, quic);
    ...
    if (ret == 0 && send_length > 0) {
        ...
        if (ret != 0) {
            if (ret == PICOQUIC_ERROR_DISCONNECTED)
                 debug("ERROR: polling a closed connection?\n", ret);
            else
                debug("ERRRORR %d !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n", ret);
